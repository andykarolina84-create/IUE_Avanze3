...
document.addEventListener("DOMContentLoaded", () => {
  const modal = document.querySelector("#modalConfiguracion .config-opciones");
  if (!modal) return; 

  // Crear opción completa
  const opcion = document.createElement("div");
  opcion.classList.add("opcion");
  opcion.innerHTML = `
    <label>Navegación por teclado:</label>
    <label class="switch">
      <input type="checkbox" id="teclas-navegacion">
      <span class="slider"></span>
    </label>
  `;

  // Insertar al final de la lista de opciones
  modal.appendChild(opcion);

  // --- Restaurar preferencia ---
  const toggleTeclas = opcion.querySelector("#teclas-navegacion");
  const guardado = localStorage.getItem("teclasNavegacion");
  if (guardado === "on") toggleTeclas.checked = true;

  // --- Guardar cambio ---
  toggleTeclas.addEventListener("change", () => {
    if (toggleTeclas.checked) {
      localStorage.setItem("teclasNavegacion", "on");
    } else {
      localStorage.setItem("teclasNavegacion", "off");
    }
  });
});
...
document.addEventListener("keydown", (event) => {
  // Bloquear si escribe en input o textarea
  const writing = document.activeElement.tagName === "INPUT" ||
                  document.activeElement.tagName === "TEXTAREA" ||
                  document.activeElement.isContentEditable;

  if (writing) return;

  // Bloquear si navegación por teclas está apagada
  const teclas = localStorage.getItem("teclasNavegacion");
  if (teclas !== "on") return;

  // Páginas vinculadas a números
  const paginas = [
    "index.html",
    "noticias.html",
    "partidos.html",
  ...
  switch (event.key) {
    case "1": window.location.href = paginas[0]; break;
    case "2": window.location.href = paginas[1]; break;
    case "3": window.location.href = paginas[2]; break;
    case "4": window.location.href = paginas[3]; break;
    case "5": window.location.href = paginas[4]; break;
  }
});
...
// =========================================
// MODAL DE EQUIPOS (AISLADA Y FUNCIONAL)
// =========================================
document.addEventListener("DOMContentLoaded", () => {

  const modalEquipo = document.getElementById("modalEquipo");
  const cerrarModalEquipo = document.querySelector(".cerrar-modal-equipo");

  if (!modalEquipo || !cerrarModalEquipo) return;

  const infoEquipos = {
    argentina: {
      nombre: "Argentina",
      bandera: "img/bandera-argentina.png",
      titulo: "Campeón del Mundo 2022",
      conf: "CONMEBOL",
      ranking: "1",
      pagina: "equipo-detalles.html"
    },
    francia: {
      nombre: "Francia",
      bandera: "img/bandera-francia.png",
      titulo: "Subcampeón Mundial",
      conf: "UEFA",
      ranking: "2",
      pagina: "equipo-detalles.html"
    },
    brasil: {
      nombre: "Brasil",
      bandera: "img/bandera-brasil.png",
      titulo: "Quinto Campeón Mundial",
      conf: "CONMEBOL",
      ranking: "3",
      pagina: "equipo-detalles.html"
    },
    inglaterra: {
      nombre: "Inglaterra",
      bandera: "img/bandera-inglaterra.png",
      titulo: "Campeón Mundial 1966",
      conf: "UEFA",
      ranking: "4",
      pagina: "equipo-detalles.html"
    }
  ...
  // CLICK EN TARJETAS
  document.querySelectorAll(".tarjeta-equipo").forEach(card => {
    card.addEventListener("click", () => {

      const id = card.dataset.equipo;

      if (!id || !infoEquipos[id]) return;

      document.getElementById("equipo-nombre").textContent = infoEquipos[id].nombre;
      document.getElementById("equipo-bandera").src = infoEquipos[id].bandera;
      document.getElementById("equipo-titulo").textContent = infoEquipos[id].titulo;
      document.getElementById("equipo-conf").textContent = infoEquipos[id].conf;
      document.getElementById("equipo-ranking").textContent = infoEquipos[id].ranking;

      document.getElementById("btnMasDetalles").onclick = () => {
        window.location.href = infoEquipos[id].pagina;
      };
